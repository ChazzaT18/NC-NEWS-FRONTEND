...
const ArticlesCard = ({ article }) => {
    ...
    return (
      <li className="article-card">
        ...
        <Link to={`/articles/${article.article_id}`} className="single-article-link"><h3 id='article-title'>{article.title}</h3></Link>
          ...
          <p><span className='article-card-bold'>Date created: </span>{`${day}-${month}-${year}`}</p>
          ...
          <Link to={`/articles/${article.article_id}`} className="single-article-link">Click here to read the full article</Link>
      </li>
    );
  };
